...
-- Expression parsing with mutual recursion
mutual
...
def parseSelect (s : ParserState) : ParserResult Statement :=
  ...
  | .ok _ s' =>
    ...
    | .ok columns s'' =>
      ...
      let (fromTable, s''') := match s''.peek with
      | some (.Keyword .FROM) =>
        match parseIdentifier (s''.advance) with
        | ParserResult.ok tableName s => (some (TableRef.Table tableName none), s)
        | ParserResult.error _ _ => (none, s'')
      | _ => (none, s'')
      
      -- Parse optional WHERE clause
      let (whereClause, s'''') := match s'''.peek with
      | some (.Keyword .WHERE) =>
        match parseExpr (s'''.advance) with
        | ParserResult.ok expr s => (some expr, s)
        | ParserResult.error _ _ => (none, s''')
      | _ => (none, s''')
      
      ParserResult.ok (.Select columns fromTable whereClause [] none none) s''''
  ...
  | .ok _ s' =>
    ...
    | .ok _ s'' =>
      ...
      | .ok tableName s''' =>
        ...
        let (columns, s'''') := match s'''.peek with
        | some .LeftParen =>
        ...
        | .ok _ s''''' =>
          ...
          | .ok _ s'''''' =>
            ...
            | .ok values s''''''' =>
              ...
              | .error msg state => .error msg state
              | .ok _ s'''''''' =>
                .ok (.Insert tableName columns [values]) s''''''''
  ...
  | .ok _ s' =>
    ...
    | .ok _ s'' =>
      ...
      | .ok tableName s''' =>
        ...
        | some (.Keyword .WHERE) =>
          match parseExpr (s'''.advance) with
          | .ok expr s => (some expr, s)
          | .error _ _ => (none, s''')
        | _ => (none, s''')
        
        .ok (.Delete tableName whereClause) s''''
