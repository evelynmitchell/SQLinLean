...
def testSimpleSelect : IO Unit := do
  ...
  | Sum.inr stmt =>
      match stmt with
      | Statement.Select _ [SelectItem.AllColumns] (some (TableRef.Table "users" none)) none [] none [] none none =>
...
def testSelectColumns : IO Unit := do
  ...
  | Sum.inr stmt =>
      match stmt with
      | Statement.Select _ cols (some (TableRef.Table "users" none)) none [] none [] none none =>
          match cols with
...
def testSelectWhere : IO Unit := do
  ...
  | Sum.inr stmt =>
      match stmt with
      | Statement.Select _ _ _ (some whereClause) _ _ _ _ _ =>
...
def testQualifiedIdentifier : IO Unit := do
  ...
  | Sum.inr stmt =>
      match stmt with
      | Statement.Select _ [SelectItem.Expr (Expr.QualifiedIdentifier "users" "name") none] _ _ _ _ _ _ _ =>
